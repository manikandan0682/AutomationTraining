...
Now you have two identical files. Remove any references to the production environment in `dev.tf`. Repeat the process, removing any references to dev in `prod.tf``
...
Edit the `prod.tf` file by commenting out the `terraform` block, the `provider` block, and the `random_pet` resource. You can comment out the configuration by adding a `/*` at the beginning of the commented out block and a `*/` at the end.
